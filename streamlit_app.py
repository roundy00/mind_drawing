...
def analyze_colors(image_file) -> dict:
    ...
    반환 예시:
    {
        "dominant_color": {"hex": "#FF8C00", "rgb": [255, 140, 0]},
        "palette": [
            {"hex": "#FF8C00", "rgb": [255, 140, 0]},
            ...
        ],
        "llm_summary": "주요 색상은 #FF8C00(따뜻한 주황 계열)이며, ..."
    }
    """
    ...
    llm_summary = (
        f"그림의 주요(지배적) 색상은 {dominant_hex} (RGB: {dominant[0]}, {dominant[1]}, {dominant[2]})입니다. "
        ...
        "objects": [
            {
                ...
                "y_center_pct": 60,
                "area_pct": 12.3,
                "position_label": "중앙 하단",
                "confidence": 0.87
            },
            ...
        ],
        "llm_summary": "그림에서 감지된 요소: 나무전체(중앙 하단, 면적 12.3%), ..."
        "plotted_image": <numpy array>  # 시각화용
    }
    """
...
def build_llm_prompt(color_result: dict, yolo_result: dict, child_age: int, child_sex: str) -> str:
...
if st.session_state.get("analysis_done"):
    ...
    # ── LLM 프롬프트 미리보기 (디버깅/개발용 - 필요 없으면 삭제 가능) ──
...
if st.session_state.get("analysis_done"):
    ...
    if prompt := st.chat_input("하고 싶은 말을 여기에 적어줘!"):
        ...
        with st.spinner("생각 중... 💭"):
            time.sleep(1)
            # 실제 LLM 연동 시: st.session_state["llm_prompt"]를 system prompt로,
            # chat_history를 messages로 넘기면 됩니다.
            response = (
